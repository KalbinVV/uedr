...
# Локальная функция отправки уведомления без циклического импорта
...
class AutoResponder:
    def __init__(self, db, task_manager, salt_mgr, script_mgr):
        """
        Инициализация авто-респондера.
        :param db: экземпляр UserDatabase
        :param task_manager: экземпляр TaskManager
        :param salt_mgr: экземпляр SaltManager (уже настроенный в app.py)
        """
        self.db = db
    ...
    def _execute_action(self, rule: dict, minion_id: str, incident_data: dict):
        ...
        def _run_task():
            ...
            try:
                ...
                logger.exception(f"[Авто] Ошибка при применении сценария '{script_name}' к '{minion_id}'")
                return {
                    "minion": minion_id,
                    "state": script_name,
                    "error": str(e)
        ...
        _send_notification("admin", f"Правило '{rule['name']}' сработало на миньоне {minion_id}", "rule", str(rule['id']))
    @staticmethod
    def _run_salt_state_wrapped(salt_mgr, minion_id: str, state_name: str):
        try:
            ...
            logger = logging.getLogger(__name__)
            logger.exception(f"[Авто] Ошибка при применении сценария '{state_name}' к '{minion_id}'")
            return {"error": str(e)}